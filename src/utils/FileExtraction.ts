...
export const SAFE_FILE_EXTENSIONS = new Set([
  ...
  "py", "rb", "java", "cs", "go", "rs", "kt",
  "php", "swift", "c", "cpp", "h", "hpp",
  "r", "jl", "dart", "scala", "lua", "pl", "pm", "sh",
  "bash", "zsh", "ps1", "psm1",
  "vue", "svelte", "html", "css", "scss", "sass", "less",
  "json", "yaml", "yml", "toml", "ini", "cfg", "conf",
  "md", "txt", "log", "xml", "proto",
]);
...
export function isSafeFile(filepath: string): boolean {
  ...
  const unsafeSegments = [
    ".ssh", ".git", ".aws", ".config", ".local",
    "credentials", "password", "secret", "token", "env",
...
export function isSuspiciousFileContent(content: string): boolean {
  const suspiciousPatterns = [
    ...
    "fetch\\(", "axios", "https?", "http\\.", "request\\(",
    "Buffer\\.", "atob\\(", "btoa\\(",
    "unserialize", "marshal",
    "chmod\\(", "chown\\(", "umask\\(",
    "mktemp", "mkdtemp",
    "subprocess\\.Popen", "subprocess\\.run", "subprocess\\.call",
    "exec\\.bash", "shutil\\.", "os\\.system", "os\\.popen", "os\\.remove", "os\\.unlink", "os\\.rename",
    "process\\.exit", "process\\.kill",
    "System\\.loadLibrary", "Runtime\\.getRuntime", "ProcessBuilder",
...
export function buildClassifierPrompt(command: string, filePath: string | null, fileContent: string | null): string {
  const lines: string[] = [
  ...
  if (filePath && fileContent) {
    lines.push("");
    lines.push(`FILE CONTEXT: The agent is trying to execute the following file "${filePath}" via this command.`);
  ...
  lines.push("");
  lines.push('Reply with ONLY valid JSON: {"allow": true or false, "reason": "short explanation"}');
