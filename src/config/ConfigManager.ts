...
function loadDefaultBlockRules(): BlockRule[] {
  ...
  return [
    ...
    },
    {
      id: 'BR-043',
      type: 'pattern',
      pattern: '\\bdd\\b\\s+(?=[^;&]*\\bif=)(?=[^;&]*\\bof=/dev/)',
      category: 'destruction',
      description: 'Raw disk write via dd (if=/of=/dev/ in any order)',
      severity: 'critical',
      enabled: true,
    },
    {
      id: 'BR-044',
      type: 'pattern',
      pattern: '\\b(?:mkfs|mke2fs)\\b',
      category: 'destruction',
      description: 'Filesystem formatting (incl. mke2fs backend)',
      severity: 'critical',
      enabled: true,
    },
    {
      id: 'BR-045',
      type: 'pattern',
      pattern: '\\b(?:shutdown|halt|poweroff)\\b',
      category: 'system-admin',
      description: 'System shutdown (incl. halt/poweroff)',
      severity: 'high',
      enabled: true,
    },
    {
