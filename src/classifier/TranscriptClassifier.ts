...
import { BlockRule, AllowException, RuleEvaluationResult } from '../types/RuleTypes';
...
export class TranscriptClassifier {
  ...
  async classify(
    ...
    try {
      ...
      if (stage1Result.prediction === 'allow') {
        this.sessionState.incrementAllow(transcript.currentToolCall, `Stage 1 fast filter: allowed`, 1);
        return createAllowResult(`Stage 1 fast filter: action allowed`, 1);
      }

      // Stage 1 predicted block — proceed to Stage 2
      const stage2Prompt = this.formatStage2Prompt(transcript, blockRules, allowExceptions);
      ...
      if (stage2Result.decision === 'deny' || ruleResult.evaluation === 'blocked') {
        const matchedRule = ruleResult.evaluation === 'blocked' ? ruleResult.matchedRule : undefined;
      ...
      // Stage 2 allowed
      this.sessionState.incrementAllow(transcript.currentToolCall, `Stage 2 CoT: allowed`, 2);
      return createAllowResult(`Stage 2 chain-of-thought: action allowed — ${stage2Result.reasoning}`, 2);
    } catch (error) {
      ...
      return this.fallbackExecutor.executeOnError(error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  prepareContext(
    ...
    for (let i = sessionHistory.length - 1; i >= 0; i--) {
      if (sessionHistory[i].role === 'tool' || sessionHistory[i].role === 'tool_result') {
        lastToolIndex = i;
        break;
      }
    }
  ...
  private handleStage1Error(error: unknown): ClassificationResult {
    if (this.fallbackExecutor.isTimeoutError(error)) {
      return this.fallbackExecutor.executeOnTimeout(error instanceof Error ? error : new Error(String(error)));
    }
    return this.fallbackExecutor.executeOnError(error instanceof Error ? error : new Error(String(error)));
  }
