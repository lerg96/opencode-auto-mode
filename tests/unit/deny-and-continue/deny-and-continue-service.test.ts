...
function createDenialResult(overrides: Partial<ClassificationResult> = {}): ClassificationResult {
  return {
    decision: 'deny',
...
describe('DenyAndContinueService', () => {
    ...
    it('should return ask-user when consecutive threshold is reached', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        sessionState.incrementDenial(
          { toolName: 'Bash', arguments: { command: 'test' }, context: { agentName: 'general', workingDirectory: '/', sessionId: 'test' } },
          `Denial ${i}`,
          'BR-001',
          1
        );
  ...
  describe('without blockRule', () => {
    it('should format message without rule info in auto-retry', async () => {
      ...
      const result = await service.handleDeny(createDenialResult({ blockRule: undefined }));
  ...
  describe('with blockRule', () => {
    it('should format message with rule info in auto-retry', async () => {
      ...
      const result = await service.handleDeny(createDenialResult({ blockRule: 'BR-005' }));
