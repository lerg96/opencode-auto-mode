...
describe('PermissionPreChecker', () => {
  ...
  describe('checkPermission', () => {
    ...
    it('should allow global allow tools for any agent', () => {
      const toolCall = createToolCall({ toolName: 'Glob', context: { agentName: 'unknown', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
    ...
    it('should deny invalid arguments', () => {
      const toolCall = createToolCall({ arguments: null as unknown as Record<string, unknown> });
      const result = checker.checkPermission(toolCall);
    ...
    it('should return not-explicitly-allowed for unknown agents not matching', () => {
      const toolCall = createToolCall({ toolName: 'Write', context: { agentName: 'unknown', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
  ...
  describe('agent exclusion', () => {
    it('should allow excluded agents immediately', () => {
      ...
      const toolCall = createToolCall({ toolName: 'Bash', context: { agentName: 'explore', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
    ...
    it('should not exclude non-excluded agents', () => {
      ...
      const toolCall = createToolCall({ toolName: 'Bash', context: { agentName: 'general', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
    ...
    it('should allow research agent to execute Bash when excluded', () => {
      ...
      const toolCall = createToolCall({ toolName: 'Bash', context: { agentName: 'research', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
    ...
    it('should be case-sensitive for agent names', () => {
      ...
      const toolCall1 = createToolCall({ toolName: 'Bash', context: { agentName: 'explore', workingDirectory: '/', sessionId: 'test' } });
      expect(checker.checkPermission(toolCall1).reason).toBe('excluded-agent');

      const toolCall2 = createToolCall({ toolName: 'Bash', context: { agentName: 'Explore', workingDirectory: '/', sessionId: 'test' } });
      expect(checker.checkPermission(toolCall2).reason).not.toBe('excluded-agent');

      const toolCall3 = createToolCall({ toolName: 'Bash', context: { agentName: 'EXPLORE', workingDirectory: '/', sessionId: 'test' } });
      expect(checker.checkPermission(toolCall3).reason).not.toBe('excluded-agent');
    });
  });
  ...
  describe('setConfigFromPlugin', () => {
    it('should load excluded agents from plugin config', () => {
      ...
      const toolCall = createToolCall({ toolName: 'Bash', context: { agentName: 'agent1', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
  ...
  describe('setAgentPermissions', () => {
    it('should allow custom agent tool permissions', () => {
      ...
      const toolCall = createToolCall({ toolName: 'Read', context: { agentName: 'custom-agent', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
  ...
  describe('permission combination', () => {
    it('should check agent permissions before global permissions', () => {
      ...
      const toolCall = createToolCall({ toolName: 'Read', context: { agentName: 'test-agent', workingDirectory: '/', sessionId: 'test' } });
      const result = checker.checkPermission(toolCall);
